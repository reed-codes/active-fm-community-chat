...
function replaceAllTextEmojis(text) {
  let allEmojis = getAllEmojisFromText(text); // TODO: get all emoji style
  ...
  if (allEmojis) {
    ...
    allEmojis.forEach(emoji => {
      ...
      if (!style) return;
      text = replaceAll(text, emoji, `<img
            style="${style}"
            data-emoji="${emoji}"
            src="${TRANSPARENT_GIF}"
          />`);
...
function getImageEmoji(emoji) {
  ...
  const emojiSpanEl = document.querySelector(`[aria-label="${emoji.native}, ${shortNames}"] > span`) || document.querySelector(`[aria-label="${emoji.id}"] > span`);
